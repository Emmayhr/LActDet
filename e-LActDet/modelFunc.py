...
# hyper-parameter
HIDDEN_SIZE = 1024  # 隐层size通常为输入size两倍
BATCH_SIZE = 8 # (8, 16, 32)
LAYERS_NUM = 2
EPOCHS_NUM = 20  # 记录每一轮的输出，取最好的结果保存
INPUT_FEATURES_NUM = 512
...
def trainLstm(train_loader, test_loader):
    log_name = "log/doc_lstm_train2.log"
    log = ApiLog(log_name)
    ...
    for epoch in range(EPOCHS_NUM):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            if val_accuracy > best_accuracy:
                ...
                torch.save(lstm_model, "checkout/lstmModel.t")    # save the model
...
def testLstm(test_loader, log):
    lstm_model = torch.load("checkout/doc2vec_lstmModel.t")
...
if __name__ == '__main__':
    ...
    log_name = "log/doc2vec_lstm_miss_test.log"
    log = ApiLog(log_name)
    log.logger.debug("------- Attack Activity test -------")
    
    np_dir = "test/miss_doc2vec/Wannacry/"
